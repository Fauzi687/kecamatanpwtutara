...
map.getView().fit([12157431.000000, -827550.000000, 12163431.000000, -824350.000000], map.getSize());
...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    ...
    for (var i = 0; i < currentFeatureKeys.length; i++) {
        if (currentFeatureKeys[i] != 'geometry' && currentFeatureKeys[i] != 'layerObject' && currentFeatureKeys[i] != 'idO') {
            ...
            var alias = layer.get('fieldAliases')[currentFeatureKeys[i]] || currentFeatureKeys[i];
            var valueHtml = '';

            if (layer.get('fieldImages')[currentFeatureKeys[i]] != "ExternalResource") {
                valueHtml = (currentFeature.get(currentFeatureKeys[i]) != null ? autolinker.link(currentFeature.get(currentFeatureKeys[i]).toLocaleString()) : '');
            } else {
                var fieldValue = currentFeature.get(currentFeatureKeys[i]);
                if (/\.(gif|jpg|jpeg|tif|tiff|png|avif|webp|svg)$/i.test(fieldValue)) {
                    valueHtml = (fieldValue != null ? '<img src="images/' + fieldValue.replace(/[\\\/:]/g, '_').trim() + '" />' : '');
                } else if (/\.(mp4|webm|ogg|avi|mov|flv)$/i.test(fieldValue)) {
                    valueHtml = (fieldValue != null ? '<video controls><source src="images/' + fieldValue.replace(/[\\\/:]/g, '_').trim() + '" type="video/mp4">Il tuo browser non supporta il tag video.</video>' : '');
                } else if (/\.(mp3|wav|ogg|aac|flac)$/i.test(fieldValue)) {
                    valueHtml = (fieldValue != null ? '<audio controls><source src="images/' + fieldValue.replace(/[\\\/:]/g, '_').trim() + '" type="audio/mpeg">Il tuo browser non supporta il tag audio.</audio>' : '');
                } else {
                    valueHtml = (fieldValue != null ? autolinker.link(fieldValue.toLocaleString()) : '');
                }
            }

            // New Custom Popup Structure
            popupText += '<div class="popup-field-row">';
            popupText += '<div class="popup-field-label">' + alias + '</div>';
            popupText += '<div class="popup-field-value">' + valueHtml + '</div>';
            popupText += '</div>';
        }
    }
    return popupText;
...
function getLayerIcon(title) {
    ...
    if (t.indexOf('jalan') !== -1) return 'fa-road';
    if (t.indexOf('sungai') !== -1) return 'fa-water';
    if (t.indexOf('rumah') !== -1 || t.indexOf('perum') !== -1 || t.indexOf('griya') !== -1 || t.indexOf('warga') !== -1) return 'fa-home';
    if (t.indexOf('sekolah') !== -1 || t.indexOf('pendidikan') !== -1 || t.indexOf('sd') !== -1 || t.indexOf('smp') !== -1 || t.indexOf('sma') !== -1 || t.indexOf('smk') !== -1 || t.indexOf('kampus') !== -1) return 'fa-graduation-cap';
    if (t.indexOf('kebun') !== -1 || t.indexOf('makam') !== -1 || t.indexOf('sawah') !== -1) return 'fa-tree';
    if (t.indexOf('batas') !== -1 || t.indexOf('desa') !== -1 || t.indexOf('dusun') !== -1 || t.indexOf('rw') !== -1) return 'fa-map-marked-alt';

    // Ikon Tambahan untuk Point
    if (t.indexOf('warung') !== -1 || t.indexOf('makan') !== -1 || t.indexOf('bakso') !== -1 || t.indexOf('mie') !== -1 || t.indexOf('soto') !== -1 || t.indexOf('resto') !== -1 || t.indexOf('cafe') !== -1 || t.indexOf('kopi') !== -1) return 'fa-utensils';
    if (t.indexOf('toko') !== -1 || t.indexOf('mart') !== -1 || t.indexOf('store') !== -1 || t.indexOf('belanja') !== -1) return 'fa-shopping-cart';
    if (t.indexOf('masjid') !== -1 || t.indexOf('mushola') !== -1) return 'fa-mosque';
    if (t.indexOf('kost') !== -1 || t.indexOf('wisma') !== -1 || t.indexOf('guest') !== -1 || t.indexOf('homestay') !== -1) return 'fa-bed';
    if (t.indexOf('laundry') !== -1) return 'fa-tshirt';
    if (t.indexOf('bengkel') !== -1 || t.indexOf('service') !== -1) return 'fa-wrench';
    if (t.indexOf('apotek') !== -1 || t.indexOf('klinik') !== -1 || t.indexOf('dokter') !== -1) return 'fa-first-aid';
    if (t.indexOf('lapangan') !== -1 || t.indexOf('gor') !== -1 || t.indexOf('gym') !== -1) return 'fa-dumbbell';

    if (t.indexOf('point') !== -1 || t.indexOf('lokasi') !== -1) return 'fa-map-marker-alt';
        ...
        if (typeof clusteredFeatures !== "undefined") {
            if (doPopup) {
                for (var n = 0; n < clusteredFeatures.length; n++) {
                    ...
                    var layerTitle = layer.get('popuplayertitle') || layer.get('title');
                    var featureName = currentFeature.get('nama') || currentFeature.get('Nama') || currentFeature.get('NAME');
                    var displayTitle = featureName || layerTitle;
                    var iconClass = getLayerIcon(displayTitle);
                    popupText += '<div class="popup-header"><i class="fas ' + iconClass + '"></i> <span class="font-semibold">' + displayTitle + '</span></div>';
                    popupText += '<div class="popup-content">';
            ...
            if (typeof clusteredFeatures !== "undefined") {
                if (doPopup) {
                    for (var n = 0; n < clusteredFeatures.length; n++) {
                        ...
                        var coord = geometry.getType() === 'Point' ? geometry.getCoordinates() : evt.coordinate;
                        var lonlat = ol.proj.toLonLat(coord);

                        popupText += '<div class="popup-header"><i class="fas ' + iconClass + '"></i> <span class="font-semibold">' + displayTitle + '</span></div>';
                        popupText += '<div class="popup-content">';
                        popupText += createPopupField(currentFeature, currentFeatureKeys, layer);
                        popupText += '<div class="mt-3 pt-3 border-t border-gray-100"><a href="https://www.google.com/maps/dir/?api=1&destination=' + lonlat[1] + ',' + lonlat[0] + '" target="_blank" class="flex items-center justify-center w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors no-underline"><i class="fas fa-directions mr-2"></i> Rute ke Lokasi</a></div>';
                        popupText += '</div>';
                    }
                }
            } else {
...
if (zoomControl) {
    topRightContainerDiv.appendChild(zoomControl);
}
//geolocate
if (typeof geolocateControl !== 'undefined') {
    topRightContainerDiv.appendChild(geolocateControl);
}
//measure
if (typeof measureControl !== 'undefined') {
    topRightContainerDiv.appendChild(measureControl);
}
//geocoder
